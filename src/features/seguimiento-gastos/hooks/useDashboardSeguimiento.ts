...
const TWELVE_PARTIDAS_REALISTAS = [
  { id: 1, codigoPartida: 34200, nombrePartida: 'Productos Químicos y Farmacéuticos', presupuestoAsignado: 70000, presupuestoEjecutado: 24500, presupuestoDisponible: 45500 },
  { id: 2, codigoPartida: 39500, nombrePartida: 'Útiles de Escritorio y Oficina', presupuestoAsignado: 20000, presupuestoEjecutado: 7000, presupuestoDisponible: 13000 },
  { id: 3, codigoPartida: 43120, nombrePartida: 'Equipo de Computación y Periféricos', presupuestoAsignado: 160000, presupuestoEjecutado: 56000, presupuestoDisponible: 104000 },
  { id: 4, codigoPartida: 25600, nombrePartida: 'Imprenta, Publicaciones y Reproducción', presupuestoAsignado: 15000, presupuestoEjecutado: 5250, presupuestoDisponible: 9750 },
  { id: 5, codigoPartida: 31100, nombrePartida: 'Alimentos y Bebidas para Personas', presupuestoAsignado: 25000, presupuestoEjecutado: 8750, presupuestoDisponible: 16250 },
  { id: 6, codigoPartida: 43110, nombrePartida: 'Equipo e Instrumental Científico de Laboratorio', presupuestoAsignado: 50000, presupuestoEjecutado: 17500, presupuestoDisponible: 32500 },
  { id: 7, codigoPartida: 34110, nombrePartida: 'Combustibles, Lubricantes y Derivados', presupuestoAsignado: 18000, presupuestoEjecutado: 6300, presupuestoDisponible: 11700 },
  { id: 8, codigoPartida: 22100, nombrePartida: 'Pasajes, Viáticos y Traslados al Interior', presupuestoAsignado: 12000, presupuestoEjecutado: 4200, presupuestoDisponible: 7800 },
  { id: 9, codigoPartida: 32100, nombrePartida: 'Papel de Escritorio y Cartulina Técnico', presupuestoAsignado: 8000, presupuestoEjecutado: 2800, presupuestoDisponible: 5200 },
  { id: 10, codigoPartida: 25200, nombrePartida: 'Estudios e Investigaciones Aplicadas DICYT', presupuestoAsignado: 35000, presupuestoEjecutado: 12250, presupuestoDisponible: 22750 },
  { id: 11, codigoPartida: 39800, nombrePartida: 'Repuestos y Accesorios Menores de Campo', presupuestoAsignado: 14000, presupuestoEjecutado: 4900, presupuestoDisponible: 9100 },
  { id: 12, codigoPartida: 43400, nombrePartida: 'Equipo de Medición y Control de Campo', presupuestoAsignado: 23000, presupuestoEjecutado: 8050, presupuestoDisponible: 14950 },
];
...
export function useDashboardSeguimiento(options: UseDashboardSeguimientoOptions = {}) {
  ...
  // Detectar usuario autenticado real desde Supabase si está disponible
  useEffect(() => {
    async function initUser() {
      ...
      if (activeUser && activeUser.id) {
        ...
        const isCoordinador = roles.some((r) => r.id === 8 || r.nombre.toLowerCase().includes('coordinador'));
        const isInvestigador = roles.some((r) => r.id === 1 || r.id === 2 || r.nombre.toLowerCase().includes('investigador') || r.nombre.toLowerCase().includes('tutor'));
        setRoleScope({
          isCoordinador,
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      ...
      const { data: rawProgramas } = await supabase
        .from('programa')
        .select('id, nombre, sigla, presupuesto, id_programa_padre, codigo_direccion_administrativa, codigo_unidad_ejecutora, codigo_programa, codigo_actividad');
  ...
  // Filtrar Programas según el selector
  const programasFiltrados = useMemo(() => {
    if (selectedProgramaId === 'all') return allProgramas;
    return allProgramas.filter(
      (p) => p.id === selectedProgramaId || p.subprogramas.some((s) => s.id === selectedProgramaId)
    );
  }, [allProgramas, selectedProgramaId]);

  // Filtrar Proyectos segun Programa Seleccionado Y/O Proyecto Seleccionado
  const proyectosFiltrados = useMemo(() => {
    let proys = allProyectos;
    ...
    return proys;
  }, [allProyectos, selectedProgramaId, selectedProyectoId]);

  // Recalcular Métricas Dinámicas según Selección
  const metrics = useMemo<DashboardMetrics>(() => {
    if (roleScope.activeScope === 'programa' && selectedProyectoId === 'all') {
      const presVigente = programasFiltrados.reduce((acc, p) => acc + p.presupuestoVigente, 0) || 1750000;
      ...
      const presVigente = proyectosFiltrados.reduce((acc, p) => acc + p.presupuestoVigente, 0) || 950000;
      const prev = presVigente * 0.12;
      const comp = presVigente * 0.18;
      const gast = proyectosFiltrados.reduce((acc, p) => acc + p.ejecutado, 0) || presVigente * 0.30;
      const disp = Math.max(0, presVigente - (prev + comp + gast));
      return {
        presupuestoVigenteTotal: presVigente,
        preventivoReservado: prev,
