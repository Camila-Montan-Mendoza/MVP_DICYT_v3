...
- Acceso permitido solo si:
  - `rolActivo === "Investigador Principal"` **y** el usuario figura en `proyecto_usuario` con `id_rol = 1` para _este_ `id_proyecto`, o
  - `rolActivo` ∈ {"Administradora DICyT", "Administrador del Sistema SIGEFI", "Resp. de Presupuestos"} (mismo `scope === "all"` de la HU 018).
- Cualquier otro caso (incluye Investigador Principal de _otro_ proyecto, o Investigador de Apoyo) → `403 Forbidden`, sin exponer datos del proyecto.
