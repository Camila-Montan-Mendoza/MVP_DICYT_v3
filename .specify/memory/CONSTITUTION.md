...
- Added/Modified principles:
  - Principle II: Renamed to "Minimalist Wizard UI & Icon-Based Design (Zero Emojis)" — Mandates minimalist wizard/stepper architecture for multi-step workflows and strictly prohibits emojis across the entire UI in favor of Lucide SVG icons.
- Added sections: None
...
Every trámite execution MUST be strictly governed by the procedure step-and-state workflow engine (`paso_flujo`, `tarea_paso_flujo`, `rol_tarea_paso_flujo`, `transicion_flujo`).

- **Actor Permissions**: Access and actions are strictly role-scoped across the defined system actors:
  ...
  4. **AD (Administradora DICyT)**: Formally approves requests & instructs issuance of purchase orders/contracts.
  ...
  14. **CP (Coordinador de Programa)**: Oversees financial tracking and execution across assigned programs and subprograms.
...
### II. Minimalist Wizard UI & Icon-Based Design (Zero Emojis)
...
- **Wizard Layout Pattern**: Multi-step trámites, complex forms, and administrative procedures MUST be structured as clean, step-by-step wizard steppers. Each wizard step MUST focus on a single clear objective, providing transparent step progress indicators, uncluttered views, and explicit back/next navigation controls.
- **Strict No-Emoji Mandate**: Emojis are STRICTLY PROHIBITED across all user interface elements (headers, titles, buttons, badges, tables, alerts, cards, steppers). All visual indicators and status metadata MUST utilize professional SVG icons from `lucide-react` exclusively.
...
- **Mockup Fidelity**: Interface layouts MUST replicate prototype mockups in `.mockups/` using clean, non-cluttered, accessible UI/UX.
...
The database schema MUST reflect the core PostgreSQL schema provided for Supabase (`docs/schema.sql`):

- Strict Foreign Keys with `ON UPDATE RESTRICT` or `CASCADE` and explicit deletion constraints (`RESTRICT` or `SET NULL`, strictly avoiding unintended cascading deletes).
- Complete traceability of state changes through `historial_tarea_tramite` with timestamp, user ID, previous task, new task, and observations.
- Document and file handling centralized via `evidencia_archivo` UUIDs for quotes, contractual documents, receipt acts, and payment vouchers (C-31, cheques, memorandums).

### VI. Strict Real Database Data & Backend Persistence Discipline (Prohibition of Mock Data)

All UI views, workflow task strategies, and API endpoints MUST actively query, render, and persist data directly using the backend and Supabase PostgreSQL database (`tramite`, `item_tramite`, `item`, `cotizacion`, `detalle_cotizacion`, `proveedor`, `documento_contractual`, `acta_recepcion_conformidad`, `solicitud_pago`, `historial_tarea_tramite`, `transicion_flujo`).
...
- **Bi-directional Supabase Persistence**: Every user operation (creating trámites, uploading quotes, awarding items, issuing purchase orders, verifying signatures, recording payments) MUST write directly to Supabase PostgreSQL tables and update workflow state via `historial_tarea_tramite`.
...
- **Core Stack**: Next.js (App Router), Supabase (Auth & Database), ShadCN UI, ShadCN Studio, Tailwind CSS, TypeScript, Lucide Icons (`lucide-react`).
...
3. **Compliance Verification**: Automated and manual reviews must verify adherence to `DESIGN.md` tokens, RBAC view dynamic rendering, strict Supabase backend persistence, wizard layout steppers, zero emoji rule, and React design pattern discipline before merging code.

**Version**: 1.3.0 | **Ratified**: 2026-07-23 | **Last Amended**: 2026-07-30
